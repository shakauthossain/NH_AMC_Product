...
function nh_rescue_plugin_if_stuck($slug){
  ...
  foreach (glob($upgrade.'/*', GLOB_ONLYDIR) as $cand){
    // if extracted folder *is* the slug or contains /$slug
    if ( basename($cand) === $slug || is_dir(trailingslashit($cand).$slug) ) {
      $src = is_dir(trailingslashit($cand).$slug) ? $cand : $cand; // pass parent to keep structure
      $res = nh_rescue_plugin_from_source($src);
...
add_action('rest_api_init', function(){
  register_rest_route('custom/v1','/rescue-ping',[
    ...
    'callback'=>fn()=> new WP_REST_Response(['ok'=>true,'mu_like_loaded'=>true],200),
  ]);
});
