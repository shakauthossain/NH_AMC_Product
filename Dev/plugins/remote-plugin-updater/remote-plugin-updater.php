...
function rpu_handle_status( WP_REST_Request $req ) {
    ...
    if ( function_exists('wp_clean_update_cache') ) { wp_clean_update_cache(); }
    ...
    // CORE
    $installed_core = get_bloginfo('version');
    $core_updates   = get_core_updates();
    $core_payload   = ['installed' => $installed_core, 'updates' => []];
    if ( is_array($core_updates) ) {
        foreach ( $core_updates as $cu ) {
            $core_payload['updates'][] = [
                'version'      => $cu->version ?? null,
                'response'     => $cu->response ?? null,
                'locale'       => $cu->locale ?? null,
                'php_version'  => $cu->php_version ?? null,
            ];
        }
    }
    ...
    if ( ! is_object($up_tr) ) { $up_tr = (object)['response'=>[]]; }

    $plugins_list = [];
    $needs_count  = 0;
    foreach ( $plugins_all as $file => $data ) {
        $slug      = explode('/', $file)[0];
        $installed = $data['Version'] ?? null;
        $has_up    = isset($up_tr->response[$file]);
        $available = $has_up ? ($up_tr->response[$file]->new_version ?? null) : $installed;
    ...
    foreach ( $themes_all as $stylesheet => $theme_obj ) {
        ...
        $has_update = isset($ut_themes->response[$stylesheet]);
        $available  = $has_update ? ($ut_themes->response[$stylesheet]['new_version'] ?? null) : $installed;
        if ($has_update) { $themes_count++; }
        $themes_list[] = [
...
function rpu_handle_update_plugins( WP_REST_Request $req ) {
    ...
    if (!empty($body['mode']) && in_array($body['mode'], ['single','bulk'], true)) { $mode = $body['mode']; }
    ...
    if ($idem_key) {
        ...
        if ($prev) { delete_transient($lock_key); return new WP_REST_Response($prev, 200); }
    }
    ...
    foreach ($plugins as $p) {
        $p = trim((string)$p);
        if (isset($all[$p])) { $requested_files[] = $p; }
        elseif (isset($file_by_slug[$p])) { $requested_files[] = $file_by_slug[$p]; }
        else { $requested_files[] = $p; }
    }
    ...
    if ($dry_run) {
        ...
        if ($idem_key) { set_transient($idem_key, $resp, 15 * MINUTE_IN_SECONDS); }
        delete_transient($lock_key);
    ...
    foreach ($requested_files as $file) {
        ...
        $results[$file] = $res;
        if ( !empty($res['ok']) && $res['action'] === 'updated' ) {
            $updated[] = $file;
        }
...
function rpu_update_one_plugin_transactional($file, array $opts) : array {
    ...
    $was_active = is_plugin_active($file) || is_plugin_active_for_network($file);
    ...
    if ( ! $src_root ) { $src_root = $unpack_to; }
    ...
    if ( $o['backup'] ) {
        ...
        // No backup: remove target folder (dangerous if copy fails — but we already staged safely)
        rpu_rrmdir($plugin_dir);
    }
    ...
    if ( $o['health_check'] ) {
        ...
        if ( is_wp_error($health) || wp_remote_retrieve_response_code($health) >= 500 ) {
            $health_ok = false;
            $health_msg = is_wp_error($health) ? $health->get_error_message() : 'HTTP '.$health['response']['code'];
        }
    }
    if ( ! $health_ok ) {
        // Rollback
...
function rpu_copy_dir($from, $to) {
    global $wp_filesystem;
    if ( ! $wp_filesystem ) { rpu_init_filesystem(); }
    if ( ! wp_mkdir_p($to) ) {
...
function rpu_move_dir($from, $to) {
    ...
    if ( method_exists($wp_filesystem, 'move') ) {
        ...
        if ( ! $ok ) { return new WP_Error('move_failed', 'Filesystem move failed.'); }
        return true;
    }
...
function rpu_wporg_plugin_info( $slug ) {
    ...
    if ( is_wp_error($res) ) { $cache[$slug] = null; return null; }
    $cache[$slug] = $res;
    return $res;
}
